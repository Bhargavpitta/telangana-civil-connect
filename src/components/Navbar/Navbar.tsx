...
const menuItems: MenuItem[] = [
  ...
  },
  {
    label: 'DOWNLOADS',
    submenu: [
      { label: 'T Ration App', href: 'https://epds.telangana.gov.in/FoodSecurityAct/', isExternal: true },
      { label: 'Digital Token Software', href: 'https://epds.telangana.gov.in/FoodSecurityAct/', isExternal: true },
      { label: 'ePoS Complaint App soon..', href: '#' },
      { label: 'CCTV Complaint App soon..', href: '#' },
      { label: 'WhatsApp Complaint App soon..', href: '#' },
    ],
  },
  {
    label: 'RTI ACTS',
    ...
    <>
      <style>
        {`
          ...
          /* Clean hover effect without extending orange line */
          ...
          <ul className="flex flex-nowrap justify-center gap-0.5 items-center">
            {menuItems.map((item) => (
              <li key={item.label} className="relative nav-item group">
                {item.submenu ? (
                  <>
                    <button className="nav-button flex items-center justify-center gap-1 whitespace-nowrap text-white hover:bg-white/10 transition-colors py-3 px-2 text-[10px] xl:text-xs 2xl:text-sm font-medium w-full">
                      {item.label}
                    ...
                    <div className="absolute top-full left-0 hidden group-hover:block z-[100] min-w-[250px] max-h-[60vh] overflow-y-auto bg-white shadow-2xl rounded-b-md custom-scrollbar">
                      {item.submenu.map((sub, index) => (
                        <button
                          ...
                          className={`flex items-center justify-between px-4 py-2.5 text-slate-700 hover:bg-blue-50 hover:text-blue-700 text-[11px] border-b border-slate-100 transition-colors w-full text-left group/item ${
                            index === 0 ? 'rounded-t-md' : ''
                          } ${index === item.submenu.length - 1 ? 'border-b-0 rounded-b-md' : ''}`}
                        >
                          <span className="group-hover/item:underline">{sub.label}</span>
                          {sub.isNew && (
