      ...
      <AnimatePresence>
        {isOpen && (
          <motion.ul
            ...
            className="dropdown-menu absolute top-full left-0 mt-1 w-80 bg-white shadow-lg rounded-md py-2 z-[100]"
            role="menu"
          >
            {loginItems.map((item) => (
              <li key={item.label} role="none">
                <Link
                  to={item.href}
                  className="dropdown-item block px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors duration-150 border-b border-gray-100 last:border-b-0"
                  role="menuitem"
